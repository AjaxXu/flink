 ...
 * and delete changes. The {@link Table} must be have unique key fields (atomic or composite)
 * or be append-only.
 *
 * <p>If the {@link Table} does not have a unique key and is not append-only, a {@link TableException}
 * will be thrown.
 *
 * <p>The unique key of the table is configured by the {@link UpsertStreamTableSink#setKeyFields(String[])}
 * method.
 *
...
public interface UpsertStreamTableSink<T> extends StreamTableSink<Tuple2<Boolean, T>> {
	 ...
	 * The method is called after {@link TableSink#configure(String[], TypeInformation[])}.
	 *
