 ...
 * state.
 * 在rowtime上join，在watermark之间先收集record，一旦确认没有需要更新的，将结果发出，然后清除状态.
 *
 ...
 * with following eventTimes of probe records accumulated: {1, 2, 5, 8, 9}, if we
...
public class TemporalRowTimeJoinOperator
	...
	private long emitResultAndCleanUpState(long timerTimestamp) throws Exception {
		...
		long lastUnprocessedTime = Long.MAX_VALUE; // 最早的未处理的probe record的eventTime
	 ...
	 * 清除right rows中小于currentWatermark的记录，但需要保留一个
	 */
	...
	private int firstIndexToKeep(long timerTimestamp, List<BaseRow> rightRowsSorted) {
		int firstIndexNewerThenTimer =
			indexOfFirstElementBiggerThanTimer(timerTimestamp, rightRowsSorted);
	...
	private int indexOfFirstElementBiggerThanTimer(long timerTimestamp, List<BaseRow> list) {
	...
	private Optional<BaseRow> latestRightRowToJoin(
			...
			long leftTime) {
		if (low > high) {
			// exact value not found, we are returning largest from the values smaller than leftTime
			if (low - 1 < 0) {
	...
	private static class RowtimeComparator implements Comparator<BaseRow>, Serializable {
		...
		private final int timeAttribute; // eventTime所在行的index
